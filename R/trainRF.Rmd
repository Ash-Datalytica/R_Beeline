...
```
 ...
 CMYJR     37502, IF1, 100    m,           %
 CMSS      37502, IF1, 100    s,           %
...
# Сохраняем все списки параметров lzk дальнейшего использования
save (list = c("importantFeatures1", "importantFeatures2", "importantFeatures3", "importantFeatures4"),
      file = "../data/features.rds")
```
...
#View (dfTrain[importantFeatures])
#str(dfTrain[importantFeatures])
nPredictors = length(importantFeatures3) - 1 # из-за y
#nPredictors = length(allFeatures)#ncol(dfTrain)-1

#nTree берем как у RF
#nTree <- 50 #500
# = 50 -  11255, IF1,      , acc= 74.8%
# = 100 - 37502, IF1, 8 min,  acc= 75.1%
# = 500 - 25 min - ошибка
...
system.time({
    set.seed(1234)
    modRRF <- train (y ~ ., method="RRF", 
                    #data = dfTrain [importantFeatures1], #
                    #data = dfTrain [importantFeatures3], # 74.02
                    data = dfTrainFull [importantFeatures3], # 74.02
                    trControl = trainControl(method = "cv", number=10, repeats=10),
                    #количетсво деревьев в лесу.
                    #странно, что при увеличении параметра точность иногда ухудшается, а иногда улучшается
                    ntree= nTree, 
                    flagReg = 1, #with regularization
                    tuneGrid = data.frame(mtry=c(
                        #ceiling(sqrt(nPredictors)/5),
                        #ceiling(sqrt(nPredictors)/4),
                        #ceiling(sqrt(nPredictors)/3),
                        9, 11, 13, 15, 17, 19, 21, 41
#                         ceiling(sqrt(nPredictors)/2),
#                         ceiling(sqrt(nPredictors)),
#                         ceiling(sqrt(nPredictors))*2
#                         , ceiling(nPredictors/3)
...
# 11255, IF1, nTree=50, 11 min, acc 73.78% (mtry=2, coefReg=1, coefImp=0)
# 11255, IF1, nTree=50, 2 min, acc 73.92% (mtry=41, coefReg=0.8, coefImp=0)
# 11255, IF3, nTree=50, 2 min ,acc 74.02% (mtry=21, coefReg=0.8, coefImp=0)
# 11255, IF3, nTree=50, 2 min ,acc 74.04% (mtry=19, coefReg=0.8, coefImp=0)
# 37502, IF3, nTree=50, 8 min ,acc= 74.44 (mtry=15, coefReg=0.8, coefImp=0)
# 37502, IF3, nTree=20, 3 min ,acc= 74.11% (mtry=21, coefReg=0.8, coefImp=0)
# 37502, IF3, nTree=10, 2 min ,acc= 73.19% (mtry=15, coefReg=0.8, coefImp=0)



modRRF
ggplot(modRRF) # показывает, что достаточно X деревьев
#modRF$finalModel
plot(modRRF$finalModel)
#varImp(modRRF)
#varImpPlot(modRRF$finalModel)
acc <- modRRF$results$Accuracy[as.numeric(rownames(modRRF$bestTune)[1])]
acc

paramRRF <- modRRF$bestTune$mtry
```

Проведем анализ сдвига/разброса на основе Случайного леса c параметром mtry =`r paramRF`.

```{r biasAndVarianceRRF, echo=FALSE, warning=FALSE, message = FALSE, cache=TRUE}
# на сокращенных данных
myPlotLearningCurve (dfTrain, dfTest, method = "RRF", targetVariable = "y", 
                     featuresList = importantFeatures3,
                     learningCurvePoints = learningCurvePoints,
                     tuneGrid = data.frame(
                                            mtry=modRRF$bestTune$mtry,
                                            coefImp=modRRF$bestTune$coefImp,
                                            coefReg=modRRF$bestTune$coefReg
                                            ),
                     title = "Random Forest with Regularization",
                     cvRepeats = 3,
                     ntree = nTree)

dfResults  <- rbind(dfResults,
                    data.frame (model="Random Forest w. Regularization", 
                          accuracy = acc, variance="Очень высокий"))
...
```{r trainGuidedRRF, echo=FALSE, warning=FALSE, message = FALSE, cache=TRUE}
#dumFeat3 <- dummyVars(y ~ ., data = dfTrainFull[importantFeatures3])
#head(predict(dummies, newdata = etitanic))
#modRRF$finalModel$feaSet
system.time({
    rrf1 <- RRF (y~., data= dfTrainFull[importantFeatures3], ntree=500, flagReg=0) # обычный лес
})
# работает в 1 поток.
# 37502, IF3, ntree=50, 17 sec, err=29.19%
# 37502, IF3, ntree=100, 33 sec, err=27.64%
# 37502, IF3, ntree=500,  3 min, err= 26.71%

rrf1

#вычисляем вектор относительной значимости параметров
impRF <- rrf1$importance
...
system.time({
    rrf2 <- RRF (y~., data=dfTrainFull[importantFeatures3], ntree=500, flagReg=1, coefReg=coefReg) # лес  с регуляризацией
})
# 37502, IF3, ntree=500,  3 min, err= 27.67%
rrf2
```
