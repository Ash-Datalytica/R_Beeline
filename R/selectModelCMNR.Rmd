...
#leraningCurvePoints <- ifelse (nCores < 11, 10, nCores-1) # Кол-во точек (моделей) для построения кривой обучения
leraningCurvePoints <- 5
dfResults <- NULL
...
#library(caret)
res <- data.frame()
res <- foreach  (m = ceiling(seq ((nrow(dfTrain)*0.3), nrow(dfTrain), length.out = leraningCurvePoints)), 
                 .combine=rbind) %dopar% {    
                     #m=84
    rows <- sample (1:nrow(dfTrain),m)    
    mod <- caret::train (y ~ ., 
...
modRF <- train (y ~ ., method="rf", 
                data = dfTrain [importantFeatures1], #74.8
...
system.time({
    ...
    #library(caret)
    res <- data.frame()
    res <- foreach  (m = seq (ceiling(nrow(dfTrain)*0.6), nrow(dfTrain), length.out = leraningCurvePoints), 
                     .combine=rbind) %dopar% {    # так параллелится на leraningCurvePoints потоков 
                         #m=39
#     for (m in seq (ceiling(nrow(dfTrain)*0.6), nrow(dfTrain), length.out = leraningCurvePoints)) { 
#         # а так параллелится по числу repeats=5
        rows <- sample (1:nrow(dfTrain),m)    
        mod <- caret::train (y ~ ., method="rf", 
                      data = dfTrain[importantFeatures1][rows, ],
                      trControl = caret::trainControl(method = "cv", number=10, repeats=5),
...
#Error in e$fun(obj, substitute(ex), parent.frame(), e$data) : worker initialization failed: Error in #confusionMatrix.default(predictions, dfTest$class, positive = "1"): the data and reference factors must have the same number of levels
#Timing stopped at: 6.993 0.519 2769.56 
...
График показывает. что модель имеет высокий разброс, то есть переобучена. Чтобы побороть этот недостаток, можно применть следующее:
...
res <- foreach  (m = seq (ceiling(nrow(dfTrain)*0.5), nrow(dfTrain), length.out=leraningCurvePoints), 
                 .combine=rbind) %dopar% {    
                     rows <- sample (1:nrow(dfTrain),m)    
