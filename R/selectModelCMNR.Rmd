...
system.time({
    ...
    #library(caret)
    res <- data.frame()
    res <- foreach  (m = seq (ceiling(nrow(dfTrain)*0.6), nrow(dfTrain), length.out = learningCurvePoints), 
                     .combine=rbind) %dopar% {    # так параллелится на learningCurvePoints потоков 
                         #m=39
#     for (m in seq (ceiling(nrow(dfTrain)*0.6), nrow(dfTrain), length.out = learningCurvePoints)) { 
...
```{r trainC50, echo=FALSE, warning=FALSE, message = FALSE, cache=TRUE, eval=T}
#View (dfTrain)
...
system.time({          
    ...
    modSVM <- train (y ~ ., method="svmPoly", 
                     ...
                     #data = dfTrain [inModelSelect, importantFeatures2],
                     data = dfTrainFull [importantFeatures2],                     
    #                 preProcess= "pca", pcaComp=10,
                     trControl = trainControl(method = "cv", number=10, repeats=3) # 5 повторов, т.к. это только выбор параметров
                       ,tuneGrid = expand.grid(
                             ##для svmLinear
...
ВОПРОС: как это сделать в случае многоклассовой модели?

```{r ROC, echo=FALSE, warning=FALSE, message = FALSE, eval=FALSE}
...
p2 <- myPlotROC (as.numeric(predicted)-1, dfTrain$y, title="Дерево решений (rpart)")
