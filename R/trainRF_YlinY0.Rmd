...
system.time({
    set.seed(1234)
    mod6 <- train (y1~x8+x23+x24+x30+x35+x37+x38+x39+x43+x44+x56+x57+x59, #y0 ~ .
                   ## Cost-sensitive methods
                    method = "C5.0Cost",# меньше столбцов!
                    #data = dfTrainFull0[colFeaturesY0], #
                    #data = dfTrainExt[importantFeatures1Y0], #
...
write.table(dfPredicted, "../data/151024_sol_CostSubstMedianRange_yLin_y0_All.csv", sep=",", quote=FALSE, row.names=FALSE)
```
